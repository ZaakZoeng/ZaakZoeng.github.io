...
<div class="projects">
  ...
  <div class="container">
    <div id="echart-cities" style="width: 100%; height: 500px;"></div>
  </div>
  <div class="grid"></div>
...
<script>
  ...
  const dataCities = {
    '2024': {
      '1月': { '深圳': 2, '杭州': 29 },
      '2月': { '宁晋': 10, '北京': 1, '海口': 2, '文昌': 1, '深圳': 6, '杭州': 9 },
      '3月': { '杭州': 29, '深圳': 2 },
      '4月': { '杭州': 26, '深圳': 4 },
      '5月': { '杭州': 26, '深圳': 5 },
      '6月': { '杭州': 24, '南昌': 2, '深圳': 4 },
      '7月': { '杭州': 31 },
      '8月': { '深圳': 3, '宁晋': 7, '北京': 3, '香港': 1, '珠海': 2, '澳门': 1, '杭州': 14 },
      '9月': { '杭州': 26, '深圳': 4 },
      '10月': { '丰城': 1, '宁晋': 6, '海口': 5, '深圳': 4, '杭州': 15 },
      '11月': { '杭州': 28, '深圳': 2 },
      '12月': { '杭州': 19, '青岛': 9, '深圳': 3 }
    },
    '2025': {
      ...
      '3月': { '青岛': 0.5, '杭州': 28.5, '上海': 2 },
      '4月': { '杭州': 17.5, '深圳': 11.5, '香港': 1 },
      '5月': { '常州': 2, '杭州': 26, '深圳': 2.5, '广州': 0.5 },
      '6月': { '深圳': 5, '杭州': 25 },
      '7月': { '杭州': 28, '深圳': 3 },
      '8月': { '杭州': 21, '深圳': 10 },
      '9月': { '深圳': 6, '广州': 1, '杭州': 20, '上海': 3 },
      '10月': { '杭州': 21, '海口': 4, '文昌': 1, '揭阳': 0.5, '深圳': 4.5 },
      '11月': { '杭州': 25.5, '深圳': 4.5 },
      '12月': { '深圳': 2.5, '杭州': 28.5 }
    },
    '2026': {
...
<script>
  ...
  var dataTravels = [
    ...
    { name: '石家庄', value: 10 },  // 转车路过（2017-）
    { name: '张家口', value: 1 },  // 旅行（2014）
    { name: '衡水', value: 5 }, // 高中3年（2014-2017）+回校宣讲（2018）
    ...
    { name: '武汉', value: 1 }, // 大学毕业去旅行（2021）
    { name: '深圳', value: 6 }, // 研究生（2021-2026）+工作
    { name: '杭州', value: 6 }, // 研究生（2021-2026）
    { name: '余姚', value: 1 }, // 参会（2021）
    { name: '香港', value: 3 }, // 和媛媛去旅行（2023）+带弟弟去旅行（2024）+办卡（2025）
    { name: '广州', value: 3 }, // 2BF聚会（2023）+2BF聚会（2025）+2BF聚会（2026）
    { name: '上海', value: 2 }, // 找蔡哥（2023）+参会（2025）
    { name: '舟山', value: 1 }, // 和室友去旅行（2023）
    { name: '北京', value: 3 }, // 找张泽培（2023）+带弟弟去旅行（2024）+博士毕业典礼（2026）
    { name: '廊坊', value: 1 }, // 找傅笠真（2023）
    { name: '诸暨', value: 1 }, // 找张伊凝（2023）
    { name: '绍兴', value: 1 }, // 和伊凝旅行（2023）
    { name: '南京', value: 1 }, // 转车路过（2023）
    { name: '泰安', value: 1 }, // 参会（2023）
    { name: '青岛', value: 3 }, // 出差（2023）+出差（2024）+出差（2025）
    { name: '千岛湖', value: 1 }, // 团建（2024）
    { name: '海口', value: 2 }, // 去媛媛家（2024）+参会（2024）+去媛媛家（2025）
    { name: '文昌', value: 2 }, // 去媛媛家（2024）+去媛媛家（2025）
    { name: '琼海博鳌', value: 1 }, // 转车路过（2024）
    { name: '澳门', value: 1 }, // 旅行（2024）+带弟弟去旅行（2024）
    { name: '珠海', value: 1 }, // 带弟弟去旅行（2024）
    { name: '丰城', value: 1 }, // 参加葛舒豪婚礼（2024）
    { name: '常州', value: 1 }, // 参加春露婚礼（2025）
    { name: '揭阳', value: 1 }, // 转车路过（2025）
    { name: '郑州', value: 1 }, // 转车路过（2026）
    { name: '开封', value: 1 }, // 参加朱宪可孙铭瑶婚礼（2026）
    { name: '邯郸', value: 1 }  // 参加妹妹婚礼（2026）
  ];

  // 注意：你原来是 [lng, lat]；Leaflet 需要 [lat, lng]
  var geoCoordMap = {
    邢台宁晋县: [114.48, 37.05],  // 河北
    石家庄: [114.48, 38.03],      // 河北
    张家口: [114.88, 40.82],      // 河北
    衡水枣强: [115.72, 37.52],    // 河北
    淄博: [118.05, 36.81],        // 山东
    辛集: [115.22, 37.94],        // 河北
    南昌: [115.89, 28.68],        // 江西
    长沙: [112.93, 28.23],        // 湖南
